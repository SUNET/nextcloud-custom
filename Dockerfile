...
RUN wget https://github.com/pondersource/nextcloud-mfa-awareness/archive/ecde2da7ab9f8ada5ca7e5976d99080e5e2b33ec.tar.gz -O /tmp/nextcloud-mfa-awareness.tar.gz \
	...
	&& cd /var/www/html/custom_apps/mfachecker && sed -i 's/composer.phar install --prefer-dist/composer.phar install --prefer-dist --no-dev' && make build \
	&& apt remove build-essential curl && apt autoremove
