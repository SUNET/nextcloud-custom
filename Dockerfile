ARG NEXTCLOUD_BASE_IMAGE_TAG=32.0.11.2-1
...
RUN wget -q https://github.com/nextcloud-releases/eurooffice/releases/download/${eurooffice_version}/eurooffice-${eurooffice_version}.tar.gz -O /tmp/eurooffice.tar.gz \
  && cd /tmp && tar xf /tmp/eurooffice.tar.gz && mv /tmp/eurooffice /var/www/html/custom_apps/
...
RUN wget -q https://github.com/nextcloud-releases/twofactor_admin/releases/download/v${twofactor_admin_version}/twofactor_admin-v${twofactor_admin_version}.tar.gz -O /tmp/twofactor_admin.tar.gz \
  && cd /tmp && tar xf /tmp/twofactor_admin.tar.gz && mv /tmp/twofactor_admin /var/www/html/custom_apps/
RUN wget -q https://github.com/nextcloud-releases/twofactor_webauthn/releases/download/v${twofactor_webauthn_version}/twofactor_webauthn-v${twofactor_webauthn_version}.tar.gz -O /tmp/twofactor_webauthn.tar.gz \
