...
class Session implements IUserSession, Emitter {
	...
	public function logClientIn($user,
		...
		$this->logger->logException(new \Exception('[GSS debug] logClientIn(user='.$user.', password='.($password === null ? 'ISNULL' : 'NOTNULL').')'), [
			'level' => ILogger::ERROR,
		]);
		...
		if ($isTokenPassword) {
			...
			$this->logger->logException(new \Exception('[GSS debug] Set app_password in session because isTokenPassword'), [
				'level' => ILogger::ERROR,
			]);
	...
	public function createSessionToken(IRequest $request, $uid, $loginName, $password = null, $remember = IToken::DO_NOT_REMEMBER) {
		$this->logger->logException(
			new \Exception('[GSS debug] createSessionToken(uid='.$uid.', loginName='.$loginName.', password='.($password === null ? 'ISNULL' : 'NOTNULL').', remember='.$remember.')'), [
			'level' => ILogger::ERROR,
		]);
		...
		try {
			...
			$this->logger->logException(new \Exception('[GSS debug] Generate token with remember = ' . $remember . ' and DO_NOT_REMEMBER is ' . IToken::DO_NOT_REMEMBER), [
				'level' => ILogger::ERROR,
			]);
			$this->tokenProvider->generateToken($sessionId, $uid, $loginName, $pwd, $name, IToken::TEMPORARY_TOKEN, $remember);
			return true;
		} catch (SessionNotAvailableException $ex) {
			$this->logger->logException(new \Exception('[GSS debug] SessionNotAvailableException'), [
				'level' => ILogger::ERROR,
			]);
	...
	public function tryTokenLogin(IRequest $request) {
		...
		if ($dbToken->getRemember() === IToken::DO_NOT_REMEMBER) {
			...
			$this->logger->logException(new \Exception('[GSS debug] Set app_password in session because token->do_not_remember'), [
				'level' => ILogger::ERROR,
			]);
		} else {
			$this->logger->logException(new \Exception('[GSS debug] DOES NOT set app_password in session because remember is ' . $dbToken->getRemember()), [
				'level' => ILogger::ERROR,
			]);
		}
